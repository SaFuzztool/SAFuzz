...
static u8 HitUnexploited;
static u8 Distance_Updated;
static u32 NumOfCoveredTargets;
static u32 NumOfUnexpolitedTarge;

static u8 is_target;
static u8 is_rare_target;

static u32 TARGET_EXPLOITED_THRESHOLD;
static u32 min_target_hit = 99999;
...
double calculate_distance() {
  ...
  if (*total_count > 0)
    res = (double) (*total_distance) / ( (double) (*total_count) + (double) (tdbbs) );
  else
    res = -1.0;
...
static int findTARGET_EXPLOITED_THRESHOLD(unsigned int num) {
    unsigned int i = 0;
    unsigned int n = 1;
...
static bool hit_rare_targets(struct queue_entry* q) {
  if (no_critical_flag || no_target_favor)
    return false;
  u32 i;
  NumOfCoveredTargets = 0;
  NumOfUnexpolitedTarge = 0;
  u8 tmp = 0;
  for (i = 0; i < targets_num; i++) {
    if (q->targets && q->targets[i]) {
      if (min_target_hit > q->targets[i]) {
        min_target_hit = target_count[i];
        tmp = findTARGET_EXPLOITED_THRESHOLD(min_target_hit);
        TARGET_EXPLOITED_THRESHOLD = 1 << tmp;
      }
      NumOfCoveredTargets += 1;
      if (target_count[i] < TARGET_LIMIT || target_count[i] < TARGET_EXPLOITED_THRESHOLD) {
        NumOfUnexpolitedTarge += 1;
      }
    }
  }
  if (NumOfUnexpolitedTarge) {
    return true;
  }
  return false;
}
...
EXP_ST u8 find_cri_position(char** argv, u8* out_buf, u32 len, int mode) {
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  perf_score = perf_score * factor * (2 - normalized_d) * (1 + NumOfUnexpolitedTarge/(1 +NumOfCoveredTargets));
...
static u8 fuzz_one(char** argv) {
  ...
  // use out_buf to do mutate
  memcpy(out_buf, in_buf, len);
  queue_cur->cbits = ck_alloc(sizeof(u32)*len);
  if (queue_cur->favored) {
    int pivotnum = find_cri_position(argv, out_buf, len, 1);
    int count = 0;
    if (pivotnum > 1) {
      for (s32 i = 0; i < UR(10); i++) {
        count++;
        u32 tmp = UR(len);
        out_buf[tmp] ^= 1 + UR(255);
        int cnt = find_cri_position(argv, out_buf, len, 2);
        if (cnt < min_target_hit && cnt > 2) {
          queue_cur->cbits[count] = tmp;
        } 
      }
    }
    queue_cur->cbits[0] = count;
  }
...
havoc_stage:
  ...
  /* We essentially just do several thousand runs (depending on perf_score)
   ...
   for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      switch (UR(15 + ((extras_cnt + a_extras_cnt) ? 2 : 0))) {
        ...
        case 11 ... 12: {
            ...
            if (!queue_cur->cbits[0]) {
              for (int i = 0; i < queue_cur->cbits[0]; i++) {
                if (del_from == queue_cur->cbits[i]) {
                  del_len = choose_block_len(temp_len - 1);
                }
              }
            }
    ...
    if (HitUnexploited || Distance_Updated) {
      reward = 1;
    }
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "SAFuzz (yeah!) " cBRI VERSION cRST "\n");
